// https://github.com/lydiahallie/algo-data-structures/blob/master/data_structures/binary_search_tree.js

https://github.com/lydiahallie/algo-data-structures/tree/master/algorithms
function Node(data) {
  this.data = data;
  this.left = null;
  this.right = null;
...
class BinarySearchTree {
  ...
  remove(data) {
    ...
    const removeNode = (node, data) => {
      ...
      if (data === node.data) {
        ...
        // the geMin function returns the value of the node on the left
        // if we pas the node to the right as an argument, the current node
  ...
  isBalanced(node) {
    ...
    // If the amount of nodes on the left is the same as the amount of nodes on the right, which means the diff is smaller than one,
  ...
  checkHeight(node) {
    ...
    // If the returned value of the checkHeight func returns -1
    // which means that there is no index for the left value, return -1
    if (left === -1) {
      return -1;
    }
